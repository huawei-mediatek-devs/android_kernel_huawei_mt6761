...
static void read_serial_num_hx83112(struct hisi_fb_data_type* hisifd,uint8_t  out[] )
{
	...
	for(i = 0; i< SN_LENGTH_HIMAX83112; i++)
	{
		...
		mipi_dsi_lread(read_value, project_id_cmd, 5, mipi_dsi0_base);
...
read_serial_num_td4336(struct hisi_fb_data_type* hisifd,uint8_t color_point_id[],uint8_t start_position,uint8_t length)
{
	int i = 0;
	uint32_t mipi_dsi0_base = 0;
	...
	mipi_dsi0_base = hisifd ->mipi_dsi0_base;
	mipi_dsi_lread(read_value_td4336, td4336_reg_cmd, READ_REG_TD4336_NUM, mipi_dsi0_base);
