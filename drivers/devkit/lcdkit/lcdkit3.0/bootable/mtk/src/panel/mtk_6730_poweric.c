...
void lcd_kit_get_bias_boost_voltage(unsigned int *vboost_val)
{
	...
	*vboost_val += 200000;
	if(*vboost_val > 6150000)
	{
		*vboost_val = 6150000;
	}
	for(i = 0;i < sizeof(vol_table) / sizeof(struct mtk_mt6731_voltage); i++) {
	...
	if (i >= sizeof(vol_table) / sizeof(struct mtk_mt6731_voltage)) {
...
int lcd_kit_display_bias_vsp_enable(struct regulate_bias_desc vspconfig)
{
	...
	}else if((1 <= delay_time)&&(delay_time < 4)) {
	...
	ret = PMU_REG_MASK(VBOOST_REG_ADDR, (unsigned char)(((vboost)|(delay_time << 6))& 0xFF), 0xFF);
	if (ret < 0)
	{
...
int lcd_kit_display_bias_vsn_enable(struct regulate_bias_desc vsnconfig)
{
	...
	ret = PMU_REG_MASK(ENABLE_REG_ADDR, (unsigned char)(VPOS_ENABLE_VAL|VNEG_ENABLE_VAL), (unsigned char)(VPOS_ENABLE_MASK|VNEG_ENABLE_MASK));
	if (ret < 0)
	{
...
int lcd_kit_display_bias_vsp_disable(void)
{
	...
	ret = PMU_REG_MASK(ENABLE_REG_ADDR, VPOS_DISABLE_VAL, VPOS_ENABLE_MASK);
	if (ret < 0)
	{
...
int lcd_kit_display_bias_vsn_disable(void)
{
	...
	ret = PMU_REG_MASK(ENABLE_REG_ADDR, VNEG_DISABLE_VAL, VNEG_ENABLE_MASK);
	if (ret < 0)
	{
		LCD_KIT_ERR("set vsp disable reg 0x%0x fail\n", ENABLE_REG_ADDR);
	}
