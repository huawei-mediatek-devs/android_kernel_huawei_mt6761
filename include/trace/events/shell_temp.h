...
TRACE_EVENT(shell_temp_read_succ,		/* [false alarm]: native interface */
    TP_PROTO(struct thermal_zone_device *tz, int temp,const char *passage_name,u32 sw_curr_temp,u32 qcom_temp,int para1),
    ...
    __field(const char *, sw_curr_temp)
    __field(const char *, qcom_temp)
    __field(const char *, para1)
    ),
...
TRACE_EVENT(shell_temp_read_fail,		/* [false alarm]: native interface */
    TP_PROTO(struct thermal_zone_device *tz, int temp,int rt_value,const char *passage_name,u32 id,u32 sensor_type,int para1),
    ...
    __field(const char *, passage_name)
    __field(const char *, id)
    __field(const char *, sensor_type)
    __field(const char *, para1)
    ),
...
TRACE_EVENT(handle_invalid_temp,		/* [false alarm]: native interface */
    ...
    __field(const char *, temperature)
    ),
    ...
    TP_printk("handle_invalid_temp : %s: idx=%d;rt_value=%d;[hisi_shell->index].temp:%d;", __get_str(thermal_zone), __entry->i,__entry->invalid_temp,__entry->temperature)
);    
